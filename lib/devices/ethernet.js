...
var setIpTableRules = function(srtEthDevice, addRule) {
  var command = "iptables -" + (addRule ? "A" : "D");
  sh.run(command + " INPUT -i " + srtEthDevice + " -j DROP");
  sh.run(command + " OUTPUT -o " + srtEthDevice + " -j DROP");
};

var Ethernet = function(srtEthDevice) {
  Ethernet.super_.call(this, srtEthDevice);
  ...
  this.addFilter('send', function(packet) {
