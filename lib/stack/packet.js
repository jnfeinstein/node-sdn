...
Packet.prototype.setType = function(type) {
  this.type = type;
  var typeOffset = this.vlan ? 18 : 14;
...
Packet.prototype.setVlan = function(vlan) {
  if (vlan) {
    if (!this.vlan) {
      this.rawPacket = this.rawPacket.insert(14, 4);
      this.rawPacket.writeUInt16BE(0x8100, 14);
      this.rawPacket.writeUInt16BE(this.type, 18);
    }
    this.vlan = vlan & 0xFFF;
    this.rawPacket.writeUInt16BE(this.vlan, 16);
  }
  else if (vlan === null && this.vlan) {
    this.rawPacket = this.rawPacket.remove(14, 4)
    this.rawPacket.writeUInt16BE(this.type, 14);
    this.vlan = null;
  }
};
